...
export default function ExposureManager({ brideId, onClose }: Props) {
  ...
  useEffect(() => { load(); }, [brideId]);

  const handleExpose = async () => {
    if (!groomId.trim()) return;
    setError('');
    try {
      await api.brides.expose(brideId, groomId.trim());
      setGroomId('');
      load();
    } catch (e: any) {
      setError(e.message);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()} dir="rtl">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-[#1B4332] dark:text-gray-100">إدارة الإتاحة للعريس</h2>
        ...
        <div className="mb-6">
          <label className="block text-sm font-medium text-[#6B7280] dark:text-gray-400 mb-2">إتاحة السجل لعريس</label>
          <div className="flex gap-2">
            <input
              type="text"
              value={groomId}
              onChange={e => setGroomId(e.target.value)}
              placeholder="أدخل معرف العريس (ID)..."
        ...
        <h3 className="text-sm font-bold text-[#1B4332] dark:text-gray-100 mb-3">
          العرسان الممنوحة لهم الإتاحة ({exposures.length})
        </h3>
              ...
              <div key={ex.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/30 rounded-lg border border-gray-200 dark:border-gray-700">
                <div>
                  <p className="text-sm font-medium text-[#1B4332] dark:text-gray-200">
                    عريس #{ex.groomId.slice(-5)}
                  </p>
                  <p className="text-xs text-[#6B7280]">
                    {ex.isActive ? '🟢 مُتاح' : '🔴 متوقف'}
                    {' | '}
