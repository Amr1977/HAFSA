...
export const addComment = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const comment = await prisma.postComment.create({
      ...
      include: { user: { select: { id: true, roles: true, subscriptionPlan: true, profile: { select: { displayName: true, photos: { where: { isPrimary: true }, take: 1 } } } } } },
    });
    if (post.userId !== req.userId) {
      const name = await getUserDisplayName(req.userId!);
...
export const getFollowers = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const followers = await prisma.follow.findMany({
      ...
      include: { follower: { select: { id: true, roles: true, subscriptionPlan: true, profile: { select: { displayName: true, photos: { where: { isPrimary: true }, take: 1 } } } } } },
      orderBy: { createdAt: 'desc' },
    });
...
export const getFollowing = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const following = await prisma.follow.findMany({
      ...
      include: { following: { select: { id: true, roles: true, subscriptionPlan: true, profile: { select: { displayName: true, photos: { where: { isPrimary: true }, take: 1 } } } } } },
      orderBy: { createdAt: 'desc' },
    });
